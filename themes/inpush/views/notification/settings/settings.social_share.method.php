...
<?php ob_start() ?>
    ...
    <div class="custom-control custom-switch mr-3 mb-3">
        <input
                ...
                id="share_vk"
                name="share_vk"
            <?= $data->notification->settings->share_vk ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label clickable" for="share_vk"><i class="fab fa-fw fa-facebook"></i> <?= language()->notification->settings->share_vk ?></label>
    </div>
    ...
    <div class="custom-control custom-switch mr-3 mb-3">
        <input
                ...
                id="share_ok"
                name="share_ok"
            <?= $data->notification->settings->share_ok ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label clickable" for="share_ok"><i class="fab fa-fw fa-twitter"></i> <?= language()->notification->settings->share_ok ?></label>
    </div>
    ...
    <div class="custom-control custom-switch mr-3 mb-3">
        <input
                ...
                id="share_telegram"
                name="share_telegram"
            <?= $data->notification->settings->share_telegram ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label clickable" for="share_telegram"><i class="fab fa-fw fa-linkedin"></i> <?= language()->notification->settings->share_telegram ?></label>
    </div>
...
<script>
    ...
    $('#settings_title').on('change paste keyup', event => {
        $('#notification_preview .inpush-social-share-title').text($(event.currentTarget).val());
    });

    $('#settings_description').on('change paste keyup', event => {
        $('#notification_preview .inpush-social-share-description').text($(event.currentTarget).val());
    });
    ...
    $('[id*="share_"]').on('change', event => {
        ...
        if($(event.currentTarget).is(':checked')) {
            $(`#notification_preview .inpush-social-share-button-${share_type}`).show();
        } else {
            $(`#notification_preview .inpush-social-share-button-${share_type}`).hide();
        }
    });
    ...
    settings_title_color_pickr.on('change', hsva => {
        ...
        $('#notification_preview .inpush-social-share-title').css('color', hsva.toHEXA().toString());
    });
    ...
    settings_description_color_pickr.on('change', hsva => {
        ...
        /* Notification Preview Handler */
        $('#notification_preview .inpush-social-share-description').css('color', hsva.toHEXA().toString());
    });
