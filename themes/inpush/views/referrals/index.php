...
<section class="container pt-5">
    ...
    <p class="text-muted"><?= sprintf(language()->referrals->invite->{'subheader_' . settings()->affiliate->commission_type}, '<strong>' . settings()->main->title . '</strong>', '<strong>' . settings()->affiliate->commission_percentage . '%</strong>') ?></p>
            ...
            <table class="table table-custom">
                ...
                <?php foreach($data->affiliate_withdrawals as $row): ?>
                    <tr>
                        ...
                        <td class="text-muted">
                            <span class="text-muted" data-toggle="tooltip" title="<?= \Inpush\Date::get($row->datetime) ?>">
                                <?= \Inpush\Date::get($row->datetime, 2) ?>
                            </span>
                        </td>
                    </tr>
                <?php endforeach ?>
