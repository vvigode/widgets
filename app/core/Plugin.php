...
class Plugin {
    ...
    public static function save_status($plugin_id, $new_status) {
        ...
        $new_settings = \Inpush\Plugin::get($plugin_id)->settings ? clone \Inpush\Plugin::get($plugin_id)->settings : (object) [];
        $new_settings->status = $new_status;

        /* Save the new config file */
        $settings_saved = file_put_contents(\Inpush\Plugin::get($plugin_id)->path . 'settings.json', json_encode($new_settings));
        chmod(\Inpush\Plugin::get($plugin_id)->path . 'settings.json', 0777);
