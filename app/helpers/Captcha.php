...
class Captcha {
    ...
    public function display() {
        ...
        if(settings()->captcha->type == 'recaptcha' && settings()->captcha->recaptcha_public_key && settings()->captcha->recaptcha_private_key) {
            ...
            echo '<input type="hidden" name="captcha" class="form-control ' . (\Inpush\Alerts::has_field_errors('captcha') ? 'is-invalid' : null) . '">';
            echo \Inpush\Alerts::output_field_error('captcha');

            if(!\Inpush\Event::exists_content_type_key('javascript', 'captcha')) {
                ob_start();
                echo '<script src="https://www.google.com/recaptcha/api.js?hl=' . Language::$language_code . '" async defer></script>';
                \Inpush\Event::add_content(ob_get_clean(), 'javascript', 'captcha');
            }
        }
            ...
            <input type="text" name="captcha" class="form-control ' . (\Inpush\Alerts::has_field_errors('captcha') ? 'is-invalid' : null) . '" placeholder="' . language()->global->captcha_placeholder . '" aria-label="' . language()->global->accessibility->captcha_input . '" required="required" autocomplete="off" />
            ' . \Inpush\Alerts::output_field_error('captcha') . '
            ';
        }
