    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(mb_strlen($_POST['password']) < 6 || mb_strlen($_POST['password']) > 64) {
                Alerts::add_field_error('password', language()->global->error_message->password_length);
            }
            ...
            if(settings()->users->blacklisted_domains && in_array($email_domain, explode(',', settings()->users->blacklisted_domains))) {
                Alerts::add_field_error('email', language()->register->error_message->blacklisted_domain);
            }
            ...
            if($country && in_array($country, settings()->users->blacklisted_countries ?? [])) {
                Alerts::add_error(language()->register->error_message->blacklisted_country);
            }
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                $plan_expiration_date       = \Inpush\Date::$date;
        ...
        $view = new \Inpush\Views\View('register/index', (array) $this);
