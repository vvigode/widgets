...
class User extends Model {
    ...
    public function get_user_by_user_id($user_id) {
        ...
        $cache_instance = \Inpush\Cache::$adapter->getItem('user?user_id=' . $user_id);
    ...
    public function get_user_by_user_id_and_token_code($user_id, $token_code) {
        ...
        /* Try to check if the store posts exists via the cache */
        $cache_instance = \Inpush\Cache::$adapter->getItem('user?user_id=' . md5($user_id) . '&token_code=' . $token_code);
    ...
    public function update_last_activity($user_id) {
        db()->where('user_id', $user_id)->update('users', ['last_activity' => \Inpush\Date::$date]);
    }
    ...
    public function create(
        ...
        $plan_expiration_date = $plan_expiration_date ?? \Inpush\Date::$date;
        $plan_trial_done = 0;
        $language = \Inpush\Language::$default_language;
        ...
        switch($user->payment_processor) {
            ...
            case 'paypal':
                ...
                $response = \Unirest\Request::post($paypal_api_url . 'v1/billing/subscriptions/' . $user->payment_subscription_id . '/cancel', $headers, \Unirest\Request\Body::json([
                    'reason' => sprintf(language()->account_plan->cancel->reason, settings()->main->title)
                ]));
